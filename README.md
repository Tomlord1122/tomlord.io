...
<img {src} {alt} {sizes} class="h-auto w-full" />
```
...
export const POST: RequestHandler = async ({ request }) => {
	...
	const res = await fetchWithTimeout(`${config.BACKEND_URL}/api/images`, {
		method: 'POST',
		body: form
	});
...
2. Set environment variables (PUBLIC\_\* and AUTH_TOKEN for sync) in Vercel
...
- Ensure PUBLIC\_\* vars are non-sensitive; secrets belong on the server only
