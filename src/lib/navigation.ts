 ...
 */
export function getBreadcrumbs(): Array<{ label: string; href: string; active: boolean }> {
	if (!browser) return [];
 ...
 */
export function getNavigationSuggestions(): Array<{ href: string; metadata: RouteMetadata }> {
	if (!browser) return [];
