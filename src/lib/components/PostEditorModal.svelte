<script lang="ts">
	...
	async function handleCreatePost() {
		...
		if (!content.trim()) { alert('Please add some content to your post.'); return; }
		if (!slug.trim()) { alert('Please enter a URL slug for your post.'); return; }
		if (!auth.token) { alert('You must be logged in to create posts.'); return; }
...
${content}`;
		...
		try {
			...
			onSaved();
			title = ''; slug = ''; content = ''; postTags = []; newTagInput = '';
			show = false;
		} catch (error) {
	...
	async function handleUpdatePost() {
		if (!postData || typeof postData.slug !== 'string' || !postData.slug.trim()) {
			...
			console.error('PostEditorModal Error: originalSlug is missing or invalid. postData.slug:', postData?.slug);
			return;
		}
		if (!title.trim()) { alert('Please enter a title for your post.'); return; }
		if (!content.trim()) { alert('Please add some content to your post.'); return; }
		if (!slug.trim()) { alert('Please enter a URL slug for your post.'); return; }
		if (!auth.token) { alert('You must be logged in to update posts.'); return; }
...
${content}`;
	...
	async function handleDeletePost() {
		if (!postData || !postData.slug) { alert('Cannot delete: post data is missing.'); return; }
		if (!auth.token) { alert('You must be logged in to delete posts.'); return; }
			...
			<form onsubmit={() => {}} class="space-y-4">
				...
				<section class="flex justify-between">
					<div class="mr-2 w-1/2 gap-3">
						<label for="post-title-input" class="mb-1 block text-sm font-medium text-gray-700">Title</label>
						<input
							type="text"
							id="post-title-input"
							bind:value={title}
					...
					<div class="w-1/2">
						<label for="post-slug-input" class="mb-1 block text-sm font-medium text-gray-700">URL Slug</label>
						<div class="flex items-center">
							<span class="rounded-l-md border border-r-0 border-gray-300 bg-gray-100 p-2 text-gray-500">/blog/</span>
							<input
								type="text"
								id="post-slug-input"
								bind:value={slug}
				...
				<div class="mt-3">
					...
					<div class="h-[500px]">
						<NotionLikeEditor
							...
							placeholder="{mode === 'create' ? 'Write' : 'Edit'} your blog post content here using Markdown. Type '/' for commands..."
						/>
					</div>
				</div>
			</form>
